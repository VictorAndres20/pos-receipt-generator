...
const writeProducts = (doc, data, posY) => {
    ...
    writeBoldPDFDoc(doc, `Cant.`, 20, posY, 18);
    writeBoldPDFDoc(doc, `Descripción`, initialPosX + 70, posY, 18);
    writeBoldPDFDoc(doc, `I.V.A. / Valor`, initialPosX + 400, posY, 18);
    posY += posYAddition;
    ...
    for (let index = 0; index < products.length; index++) {
        ...
        totalPrice += total;
        writePDFDoc(doc, `${quantity}`, 10, posY, 20);
        writePDFDoc(doc, `${product?.descrip}`, initialPosX + 20, posY, 19);
        writeBoldPDFDoc(doc, ` - /  ${formatToUSD(total)}`, initialPosX + 430, posY, 22);
    ...
    writePDFDoc(doc, `${formatToUSD(totalPrice)}`, initialPosX + 400, posY);
    posY = nextLine(doc, posY);
    writeBoldPDFDoc(doc, `Valor descuento:`, initialPosX + 200, posY);
    writePDFDoc(doc, `${formatToUSD(0)}`, initialPosX + 400, posY);
    posY = nextLine(doc, posY);
    writeBoldPDFDoc(doc, `Sub Total:`, initialPosX + 200, posY, 20);
    writePDFDoc(doc, `${formatToUSD(totalPrice)}`, initialPosX + 400, posY, 22);
    posY = nextLine(doc, posY);
    ...
    writeLinePDFDoc(doc, posY);
    posY = nextLine(doc, posY);
    writeBoldPDFDoc(doc, `TOTA A PAGAR:`, initialPosX + 200, posY, 20);
    writeBoldPDFDoc(doc, `${formatToUSD(totalPrice)}`, initialPosX + 400, posY, 22);
    posY = nextLine(doc, posY);
    writeLinePDFDoc(doc, posY);
    posY = nextLine(doc, posY);
    writeBoldPDFDoc(doc, `RECIBE`, initialPosX + 200, posY);
    writeBoldPDFDoc(doc, `${formatToUSD(moneyGot)}`, initialPosX + 400, posY);
    posY = nextLine(doc, posY);
    writeBoldPDFDoc(doc, `CAMBIO:`, initialPosX + 200, posY);
    writeBoldPDFDoc(doc, `${formatToUSD(moneyGot - totalPrice)}`, initialPosX + 400, posY);
    posY = nextLine(doc, posY);
